...
static NSString * const CC_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const CCRouterParameterURL = @"CCRouterParameterURL";
NSString *const CCRouterParameterCheckMethod = @"CCRouterParameterCheckMethod";
NSString *const CCRouterParameterCompletion = @"CCRouterParameterCompletion";
NSString *const CCRouterParameterUserInfo = @"CCRouterParameterUserInfo";

NSString *const CCRouterProtocolMethondRegister = @"reRegisterAll";
NSString *const CCRouterProtocolMethondDeregister = @"deregisterAll";
NSString *const CCRouterProtocolMethondCanRegister = @"canRegisterUrl:";
...
+ (void)deregisterTarget:(NSString *)targetName {
    [[CCMediator sharedInstance] performTarget:targetName action:CCRouterProtocolMethondDeregister params:nil shouldCacheTarget:NO];
}
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    //在config中拦截的字段，可自行设置
    if (parameters[CCRouterParameterCheckMethod]) {
        SEL selector = NSSelectorFromString(parameters[CCRouterParameterCheckMethod]);
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:userInfo];
        [dict setValue:completion forKey:@"block"];
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        [[CCRouterConfig sharedInstance] performSelector:selector withObject:dict];
        return;
    }
    ...
    if (parameters) {
        CCRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[CCRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[CCRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
+ (void)openURL:(NSString *)URL targetName:(NSString *)targetName withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion {
    if (![self canOpenURL:URL]) {
        BOOL isCanRegister = [[CCMediator sharedInstance]performTarget:targetName action:CCRouterProtocolMethondCanRegister params:@{@"url":URL} shouldCacheTarget:NO];
        if (isCanRegister) {
            [[CCMediator sharedInstance] performTarget:targetName action:CCRouterProtocolMethondRegister params:nil shouldCacheTarget:NO];
        }
    }
    [self openURL:URL withUserInfo:userInfo completion:completion];
}
...
{
    CCRouter *router = [CCRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
    CCRouterObjectHandler handler = parameters[@"block"];
...
- (void)addURLPattern:(NSString *)URLPattern andHandler:(CCRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(CCRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly
{
    ...
    parameters[CCRouterParameterURL] = url;
    
    NSMutableDictionary* subRoutes = self.routes;
    NSArray* pathComponents = [self pathComponentsFromURL:url];
    
    if(pathComponents.count > 2) {
        NSString *checkString = [pathComponents objectAtIndex:2];
        if ([[[CCRouterConfig sharedInstance] checkArray] containsObject:checkString]) {
            parameters[CCRouterParameterCheckMethod] = [[CCRouterConfig sharedInstance] selectorStringForKey:checkString];
            return parameters;
        }
    }
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:CC_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:CC_ROUTER_WILDCARD_CHARACTER];
        }
    }
...
- (NSString *)encodeUrl:(NSString *)url {
    ...
    NSString *encodeString = [url stringByAddingPercentEncodingWithAllowedCharacters:allowedCharacters];
    return encodeString;
}
